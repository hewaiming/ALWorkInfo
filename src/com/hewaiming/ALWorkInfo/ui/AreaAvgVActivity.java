...
public class AreaAvgVActivity extends Activity implements HttpGetListener, OnClickListener, OnScrollListener {
	private Spinner spinner_area, spinner_PotNo, spinner_beginDate, spinner_endDate;
	private Button findBtn, backBtn;
	private TextView tv_title, tv_Total, tv_FootTitle;
	private int areaId = 11;
	private ArrayAdapter<String> Area_adapter, Date_adapter;
	private HttpPost_BeginDate_EndDate http_post;
	private String url = ":8000/scgy/android/odbcPhP/GetAreaAvgV_DayTable.php";
	private String BeginDate, EndDate;
	private List<String> dateBean = new ArrayList<String>();

	private List<AvgV_Area> listBean = null;
	private HSView_AreaAvgVAdapter mAdapter = null;

	private LinearLayout showArea = null;
	private View layout_areaAvgV;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_area_avgv);
		MyApplication.getInstance().addActivity(this);
		layout_areaAvgV = findViewById(R.id.Layout_AreaAvgV);
		GetDataFromIntent();
		mContext = this;
		init_area();
		init_date();
	...
	private void init_footer() {
		...
		lv_areaAvgV.addFooterView(footView);
	...
	private void init_HSView() {
		...
		lv_areaAvgV = (ListView) findViewById(R.id.lv_AreaAvgV);
		lv_areaAvgV.setOnTouchListener(new ListViewAndHeadViewTouchLinstener());
		lv_areaAvgV.setCacheColorHint(0);
		lv_areaAvgV.setOnScrollListener(this);
		/*
	...
	public void GetDataUrl(String data) {
		if (data.equals("")) {
			Toast.makeText(getApplicationContext(), "没有获取到[平均电压]数据，可能无符合条件数据！", Toast.LENGTH_LONG).show();
			tv_Total.setText("未获取到平均电压数据 ");
			if (listBean != null) {
				if (listBean.size() > 0) {
			...
			mAdapter = new HSView_AreaAvgVAdapter(this, R.layout.item_hsview_area_avgv, listBean, mHead);
	...
	class ListViewAndHeadViewTouchLinstener implements View.OnTouchListener {
		...
		public boolean onTouch(View arg0, MotionEvent arg1) {
			...
			HorizontalScrollView headSrcrollView = (HorizontalScrollView) mHead
					.findViewById(R.id.horizontalScrollView_AreaAvgV);
	...
	private LineData getLineDatafromDayTable(int count,Double range, String Area_V,float AvgV) {
       //count 表示列表数据总数量  Area_V 图表名
		...
		// y轴的数据集合
		LineDataSet mlineDataSet = new LineDataSet(yValues, Area_V);/* 显示在比例图上 */
