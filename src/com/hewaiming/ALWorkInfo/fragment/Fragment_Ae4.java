...
public class Fragment_Ae4 extends Fragment implements OnScrollListener {
	private View mView;
	...
	public Fragment_Ae4(List<Map<String, Object>> jXList,List<String>mDateBean,String mip,int mport) {
		this.JXList=jXList;
	...
	public void onAttach(Activity activity) {
		...
		mActivity.setHandler_Ae4(mHandler_Ae);
	}
	...
	private void init_adapter(Object data) {
		if (data.equals("")) {
			Toast.makeText(this.getActivity(), "没有获取到[效应情报表]前三次AE，可能无符合条件数据！", Toast.LENGTH_LONG).show();
			if (listBeanAe != null) {
				if (listBeanAe.size() > 0) {
					listBeanAe.clear(); // 清除LISTVIEW 以前的内容
					Ae_Adapter.onDateChange(listBeanAe);
				}
			}
		} else {
			...
			Ae_Adapter = new HSView_AeRecAdapter(this.getActivity(), R.layout.item_hsview_ae_rec, listBeanAe,
					mHeadAe);
			lvAe.setAdapter(Ae_Adapter);
		}
